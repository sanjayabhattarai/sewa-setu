...
export async function POST(req: Request) {
  try {
    // 1. Lazy load dependencies to avoid build errors
    const { db } = await import("@/lib/db");
    
    // 2. Get the raw body as text for signature verification
    ...
    try {
      // 3. VERIFY THE REQUEST (Security Check)
    ...
    if (event.type === "checkout.session.completed") {
      ...
      try {
        // 5. SAVE TO DATABASE (Reliability Check)
