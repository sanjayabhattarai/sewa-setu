...
export function AvailabilityModal({ doctor, slots, isOpen, onClose, onBook }: Props) {
  const [selectedSlot, setSelectedSlot] = useState<ApiAvailabilitySlot | null>(null);

  // ✅ Reset internal state when modal closes OR doctor changes
  useEffect(() => {
    if (!isOpen) {
  ...
  useEffect(() => {
    // when switching doctors, never keep old selection
    setSelectedSlot(null);
  }, [doctor.id]);

  if (!isOpen) return null;

  // ✅ Only active slots + stable sort
  ...
  const sortedTimes = useMemo(() => {
    ...
    for (const s of activeSlots) allTimes.add(s.startTime);
          ...
          {activeSlots.length === 0 ? (
            ...
            <div className="w-full overflow-x-auto">
              <div className="min-w-max">
                ...
                >
                  <div className="px-3 py-3 text-xs font-semibold text-slate-600 uppercase">
                    Time
                  </div>
                  {Array.from({ length: 7 }).map((_, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`text-xs ${
                            hasSlots ? "text-blue-600" : "text-slate-500"
                          }`}
                        >
                              ...
                              >
                                ...
                                <div
                                  className={`font-semibold text-xs ${
                                    isOnline ? "text-blue-900" : "text-emerald-900"
                                  }`}
                                >
                                ...
                                <div
                                  className={`text-xs ${
                                    isOnline ? "text-blue-700" : "text-emerald-700"
                                  }`}
                                >
